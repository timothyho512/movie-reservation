...
public class CheckoutService {
    ...
    public CheckoutLockResponse lockSeats(CheckoutLockRequest request) {
        ...
        // check that showtime, seats, and user (if userId provided) exist in the database
        existenceValidation(showtime, seats, user);
    ...
    public CheckoutConfirmResponse confirmCheckout(CheckoutConfirmRequest request) {
        // 1. Validate request (showtimeId, seatIds, userId/guestEmail, sessionId)
        // 2. Verify the lock is still valid and matches the requested seats
        // 3. If valid, create a reservation in the database and mark seats as reserved
        // 4. Clear the lock
        // 5. Return reservation confirmation details to the client
        // 6. If lock is invalid or expired, return an error response indicating the issue
        return new CheckoutConfirmResponse();
    }


    // Helper method to validate lock request
    private void validateRequest(CheckoutLockRequest request) throws IllegalArgumentException {
    ...
    private void existenceValidation(Showtime showtime, List<Seat> seats, User user) {
        // Validate that showtimeId and seatIds exist in the database
        if (!showtimeRepository.existsById(showtime.getId())) {
            throw new ResourceNotFoundException("Showtime not found with id: " + showtime.getId());
        }

        // Validate that all seatIds exist in the database
        for (Long seatId : seats.stream().map(Seat::getId).toList()) {
            if (!seatRepository.existsById(seatId)) {
                throw new ResourceNotFoundException("Seat not found with id: " + seatId);
            }
        }

        // If userId is provided, validate that it exists in the database
